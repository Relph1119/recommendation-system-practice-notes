...
"""
@author: HuRuiFeng
@file: friend_suggestion_out_in_cosine.py
@time: 2020/5/7 19:40
...
class FriendSuggestionOutInCosine:
            ...
            for v in self.G[u]:
                if v != user and v not in user_friends:
                    if v not in user_sim:
                        user_sim[v] = 0
                    user_sim[v] += 1
        user_sim = {v: user_sim[v] / math.sqrt(len(self.G[user]) * len(self.GT[v])) for v in user_sim}
