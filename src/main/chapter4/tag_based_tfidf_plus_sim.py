...
"""
@author: HuRuiFeng
@file: tag_based_tfidf_plus_sim.py
@time: 2020/4/23 19:27
@project: recommendation-system-practice-notes
@desc: 在TagBasedTFIDF++算法基础上，考虑数据的稀疏性，添加标签的相似度
"""
...
class SimTagTFIDF(TagBasedTFIDFPlus):
    ...
    def _build_tag_vec(self, train):
        ...
        :param train: tuple(int,int,int) 训练集(用户ID,item_id,tag_id)
        """
    ...
    def tag_similarity(tag1, tag2):
        """计算两个标签的相似度
        :param tag1: dict {int, int) 标签1，dict(item_id: 被标记的个数)
        :param tag2: dict {int, int} 标签2，格式同标签1
        :return: float 相似度
        """
        similarity = 0.
    ...
    def tag_vec_report(self, tops=10):
        ...
        :param tops: int 取热度最大的前多少个
        :return: pd.DataFrame 相似性矩阵
            ...
            for j, (tag2_id, tag2_items) in enumerate(tag_vec_item):
                ...
                # 未计算过
                    ...
                    similarity_matrix[tag1_id].append(
                        self.tag_similarity(tag1_items, tag2_items))
    ...
    def _recommend_user(self, user):
        ...
        # 如果用户标记的标签过少则推荐相关的标签
        if len(user_tags) < self.recommend_least_tag:
            recommends = dict()
            ...
            recommends = sorted(recommends.items(), key=itemgetter(1), reverse=True)[:self.recommend_least_tag]
            recommends = dict(recommends)
            self.user_tag[user] = {**user_tags, **recommends}

        # 综合相关标签信息推荐
        return super()._recommend_user(user)
